...
with MPRester(api_key='ByjiPidDAqBLRBjr0gbAiYq1SOIL03u5') as mpr:
    ...
    for i, dif in enumerate(final_stats):
        if have_active_set:
            print(
                f'{ids[i]}: mp above hull: {mp_energies_final[i]}, ace above hull: {ace_energies_final[i]}, diff: {dif}, gamma: {uncertainties[i]}'
            )
        else:
            print(
                f'{ids[i]}: mp above hull: {mp_energies_final[i]}, ace above hull: {ace_energies_final[i]}, diff: {dif}'
            )
