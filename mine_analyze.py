...
for i,item in enumerate(mined_ids):
    ...
    for i,c in enumerate(calculations):
        ...
        # structures
        try:
            structure = response_json['data']['archive']['results']['material']['structural_type']
        except (KeyError, TypeError):
            structure = None
        ...
        # crystal system
        try:
            crystal_system = response_json['data']['archive']['results']['material']['symmetry']['crystal_system']
        except (KeyError, TypeError):
            crystal_system = None
        ...
        # k line density
        try:
            k_line_density = response_json['data']['archive']['results']['method']['simulation']['precision']['k_line_density']
        except (KeyError, TypeError):
            k_line_density = None

        try:
            program_name = response_json['data']['archive']['results']['method']['simulation']['program_name']
        except (KeyError, TypeError):
            program_name = None
        ...
        try:
            scf = response_json['data']['archive']['results']['method']['simulation']['dft']['scf_threshold_energy_change']
        except (KeyError, TypeError):
            scf = None
        ...
        # size
        try:
            size = response_json['data']['archive']['results']['material']['topology']['n_atoms']
        except (KeyError, TypeError):
            size = len(ase_atoms)
